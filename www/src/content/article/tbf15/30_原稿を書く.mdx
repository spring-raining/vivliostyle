...
このようにnpm scriptを実行するコマンドは、一般的には `npm run …` のように紹介されています。ただ、<Anchor article="tbf15/20_node-js-npmを使った開発とファイル構成" secTitle="開発環境を用意する" />でインストールしたpnpmでは、npm scriptを `pnpm …` と短縮して実行する機能が備わっているため、以降はこちらのコマンドで紹介しています。
...
それでは、実際に原稿を執筆していきましょう。<Anchor article="tbf15/30_原稿を書く" secTitle="Webと出版物、コンテンツの粒度の違い" />で最小単位のコンテンツはWebページであることを紹介しましたが、そのWebページの本体はAstroのContent Collectionとして保存します。では、Content Collectionにコンテンツを追加するにはどうすればよいでしょうか？
...
このように、AstroファイルのHTMLは、JavaScript部分で定義されたコンポーネントや変数を埋め込めるようになっています。埋め込みのための書式は、<Anchor article="tbf15/30_原稿を書く" secTitle="MDXで執筆する" />でも紹介したJSXというフォーマットから導入されたものです。本書に出てくる書式でわからないところが出てきた場合は、ドキュメントを読んでみましょう。
...
ところで、このコンポーネントではCSSファイルをコンポーネントスクリプト内でインポートしていますが、コンポーネントテンプレート内で `<link rel="stylesheet" href="/styles/global.css" />` のように直接インポートしない理由について気になる方がいたかもしれません。この違いについては、<Anchor article="tbf15/50_cssでデザインする" secTitle="Web向けのCSSを編集する" />で詳しく紹介します。
...
一つレイアウトコンポーネントの便利な使い方を紹介します。<Anchor article="tbf15/30_原稿を書く" secTitle="コンポーネントを使う・作る" />で紹介した通り、AstroではMarkdownファイルもコンポーネントの一種です。そのため、 `www/src/pages` にMarkdownファイルを置くと、その内容がそのままWebページになります。
...
- `www/src/pages/modules/[...file].ts`：外部npm package提供用のエンドポイント（<Anchor article="tbf15/50_cssでデザインする" secTitle="テーマを読み込む" />で改めて紹介します）
...
<Footnote>AstroのSSRモードを有効にすると `getStaticPaths` は不要になりますが、それと同時にSSRモードではMPAとしてビルドすることができなくなるため、<Anchor article="tbf15/10_webと出版物のハイブリッド開発とは" secTitle="Astroを使う理由" />で述べた通りVivliostyleで読み込める形式でビルドするという目標が達成できなくなります。</Footnote>
。
...
`type Props = …` の部分は、<Anchor article="tbf15/30_原稿を書く" secTitle="コンポーネントを使う・作る" />でも紹介したコンポーネントのpropsの定義です。この部分で、 `getCollection` 関数で受け取ったデータをそのままpropsとして受け渡すことを定義しています。このpropsには `render` という関数が用意されており、この関数を `await post.render()` の部分で実行することで、コンテンツをレンダリングした結果を `Content` 変数として受け取っています。
...
ところで、<Anchor article="tbf15/30_原稿を書く" secTitle="コレクションを追加する" />ではarticleコレクション以外にauthorコレクションを追加しましたが、このコンテンツはまだWebページ上に表示されていません。あるコンテンツに関連付けられた別のコンテンツを参照する方法を考えます。
