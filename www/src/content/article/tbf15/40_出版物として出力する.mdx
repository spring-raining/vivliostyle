...
<Anchor article="tbf15/30_原稿を書く" secTitle="Webページをビルドする" />で紹介した通り、 `www/dist` にはAstroでビルドしたWebページのためのファイルが保存されています。 `copyAsset` の設定により、これらのファイルは全て出版物にも含めることができます。この仕組みを理解するために、 `www/dist` に生成されたファイルを出版物に利用してみます。
